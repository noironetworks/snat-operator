						...
						if utils.MatchLabels(item.Spec.Selector.Labels, service.ObjectMeta.Labels) {
							requests = append(requests, reconcile.Request{
								NamespacedName: types.NamespacedName{
									...
									Name:      "snat-policyforservicepod$" + item.ObjectMeta.Name + "$" + pod.ObjectMeta.Name + "$" + service.Spec.ExternalIPs[0],
								},
							})
							break Loop
						}
...
func (h *handleService) Map(obj handler.MapObject) []reconcile.Request {
	...
	requests = append(requests, reconcile.Request{
		NamespacedName: types.NamespacedName{
			Name: "snat-service$" + service.ObjectMeta.Namespace + "$" + service.ObjectMeta.Name + "$" + service.Spec.ExternalIPs[0],
		},
	})
	return requests
}
